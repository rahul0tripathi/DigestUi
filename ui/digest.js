...
exp.get('/api/eaters', (req, res) => {
    res.send(client_apps)
})
exp.post('/api/fire', async (req, res) => {
...
exp.listen(config[ENV.env].port, () => {
...
function createWindow () {
  ...
  let win = new elec.BrowserWindow({
    ...
    icon: __dirname+'./public/img/logo-digest.ico'
  })
  win.maximize()
  win.show()
  // and load the index.html of the app.
  win.loadFile(path.join(__dirname,'./public/views/home.html'))
  win.webContents.openDevTools()
}

elec.app.whenReady().then(createWindow)
console.log(ENV)