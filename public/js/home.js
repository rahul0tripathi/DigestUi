...
$(document).ready(() => {
    ...
    var default_headers = {
        'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ0b2tlblBhcmFtcyI6eyJpZCI6IjAwMTA2ZDRlLWFmZjEtNGI3My04MWY2LWNmZTBhZDVhNWNjYSIsInBob25lIjo2Mjk0NDU4NTgzfSwiaWF0IjoxNTc4ODk1MTA3fQ.cS3oldXH1gnNRLlRLEvp1dZbmxCn_HbNWNq5ollus-w',
...
`
    ...
    const digest = axios.create({
        baseURL: `http://${window.location.hostname}:5432/api`,
    });
    function display(client){
    digest.get('/eaters').then(res => {
        Object.keys(res.data).forEach(key => {
            _host = `http://${res.data[key].host}:${res.data[key].port}`
           if(client){holders.client_info.prepend(`
